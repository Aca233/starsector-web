...
export const flareSystems = (Object.keys(data) as (keyof typeof data)[]).map(id => {
  ...
  return nativeSystem(id, {
    description: '按舰体原生发射口投放热诱弹；敌方可见制导导弹进入300范围时按原版概率与ECCM判定，失败后不重复重骰。',
    ...
    ...systemWeaponLauncher(number('burst size'), (ship, slot, world) => {
      ...
      const p: Projectile = {
        id: world.combatRandom.next(), sourceShipId: ship.id, slotId: slot.slotId, isPlayer: ship.isPlayer,
        ...
        isRocket: true, isGuided: false, isFlare: true, renderTargetIndicator: true,
        flareLife: number('flight time'), flareMaxLife: number('flight time'),
        flareBehavior: {mode: source.missileType === 'FLARE_SEEKER' ? 'SEEKER' : 'STANDARD',
