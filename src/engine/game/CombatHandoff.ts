...
export function captureFleetMember(ship: Ship, memberId: string): FleetMember {
  return decodeFleetMember({
    id: memberId, hullId: ship.spec.id, status: ship.isDead || ship.hullHp <= 0 ? 'destroyed' : 'ready',
    hullFraction: ship.isDead ? 0 : fraction(ship.hullHp / ship.maxHullHp),
    combatReadiness: fraction(ship.currentCR),
    armor: {
    ...
    hullMods: [...(ship.spec.hullMods ?? [])], captainSkills: { ...ship.spec.captainSkills },
...
export function fleetMemberSpec(member: FleetMember): ShipSpec {
  ...
  return { ...base, hullMods: member.hullMods ?? base.hullMods, captainSkills: member.captainSkills ?? base.captainSkills,
...
export class CombatHandoff {
  ...
  public collect(session: CombatSession): CombatOutcome {
    ...
    if (this.request.kind === 'fleet' && session.engine.capitalShips.some(ship => ![...this.bindings.values()].includes(ship))) {
    ...
    const capture = (roster: FleetMember[]) => roster.map(member => {
      ...
      if (!ship || !session.engine.capitalShips.includes(ship)) throw new Error('战斗实例已更换，不能写回旧出击。');
      return captureFleetMember(ship, member.id);
    });
    return {
