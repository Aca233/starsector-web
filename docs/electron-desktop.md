...
- 默认以**单机模式**进入主菜单，只监听本机。点击主菜单的局域网 / Steam 入口或原生“游戏”菜单后，自动在后台准备对应服务并进入页面，不需要手动切换模式。首次从非 Steam 启动进入 Steam 模式时，会在通过未保存编辑保护后自动重启一次，以便在图形设备创建前初始化浮层；已按 Steam 模式启动时直接进入，不重复重启；只有页面确实报告未保存编辑时才保留离开保护。不同联机入口之间仍会停止原有后台、结束其房间，不会同时托管 LAN 与 Steam 对局。
