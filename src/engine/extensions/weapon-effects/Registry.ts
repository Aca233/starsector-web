...
import { moteHit } from './MoteHit';
export const weaponEffects = new DefinitionRegistry<WeaponEffectDefinition>('weapon effect', d=>{
  validateResources(d.resources); validateHooks(d,['beam','hit','hitProjectile','advance']);
  if (!d.beam && !d.hit && !d.hitProjectile && !d.advance) throw new Error(d.id + ': no effect hook');
});
...
weaponEffects.register(moteHit);
