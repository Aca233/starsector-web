  ...
  public update(dt: number, ctx: ShipStatusContext) {
    ...
    for (const ship of ships) {
      ...
      if (ship.flux.isOverloaded) {
        if (ctx.random.next() < dt * 18) {
          const r1 = (ctx.random.next() - 0.5) * ship.spec.collisionRadius * 1.3;
          const r2 = (ctx.random.next() - 0.5) * ship.spec.collisionRadius * 1.3;
          const start = ship.pos.clone().add(new Vector2(r1, r2).rotate(ship.facingRad));
          const end = start.clone().add(new Vector2(
            (ctx.random.next() - 0.5) * 60,
            (ctx.random.next() - 0.5) * 60
          ));
      ...
      if (ship.flux.isVenting) {
        ...
        if (ctx.random.next() < dt * 14 * arcRate) {
          const r1 = (ctx.random.next() - 0.5) * ship.spec.collisionRadius * 1.1;
          const r2 = (ctx.random.next() - 0.5) * ship.spec.collisionRadius * 1.1;
          const start = ship.pos.clone().add(new Vector2(r1, r2).rotate(ship.facingRad));
          const end = start.clone().add(new Vector2(
            (ctx.random.next() - 0.5) * 45,
            (ctx.random.next() - 0.5) * 45
          ));
          ctx.fx.spawnEmpArc(start, end, {
            coreColor: [255, 255, 255],
      ...
      if (damageRatio > 0.35 && ctx.random.next() < dt * damageRatio * 16) {
        const ventOff = new Vector2(
          (ctx.random.next() - 0.5) * ship.spec.collisionRadius * 0.8,
          (ctx.random.next() - 0.5) * ship.spec.collisionRadius * 0.8
        ...
        ctx.fx.contrails.push({
          ...
          vel: Vector2.fromAngle(
            ship.facingRad + Math.PI + (ctx.random.next() - 0.5) * 1.2,
            25 + ctx.random.next() * 35
          ).addScaled(ship.vel, 0.3),
          life: 0.9 + ctx.random.next() * 0.6,
          maxLife: 1.5,
          size: 10 + ctx.random.next() * 8,
          maxSize: 32 + ctx.random.next() * 18,
          alpha: 0.75 * damageRatio,
          rotation: ctx.random.next() * Math.PI * 2,
          color: [35, 35, 40]
        });

        if (ship.hullHp < ship.spec.hitpoints * 0.45 && ctx.random.next() < 0.6) {
          ctx.fx.particles.push({
            pos: ventPos.clone(),
            vel: Vector2.fromAngle(ctx.random.next() * Math.PI * 2, 40 + ctx.random.next() * 60).addScaled(ship.vel, 0.5),
            life: 0.2 + ctx.random.next() * 0.25,
            maxLife: 0.45,
            size: 3 + ctx.random.next() * 4,
            color: [255, 140 + ctx.random.next() * 80, 30],
            alpha: 1.0
          });
        }
      }
      ...
      if (ship.armor && ctx.random.next() < dt * 6) {
        const armor = ship.armor;
        const randC = Math.floor(ctx.random.next() * armor.cols);
        const randR = Math.floor(ctx.random.next() * armor.rows);
        ...
        if (cellVal < armor.maxCellArmor * 0.25) {
          ...
          ctx.fx.contrails.push({
            pos: worldPos,
            vel: Vector2.fromAngle(ship.facingRad + Math.PI + (ctx.random.next() - 0.5) * 1.5, 15 + ctx.random.next() * 20).addScaled(ship.vel, 0.4),
            life: 0.5 + ctx.random.next() * 0.3,
            maxLife: 0.8,
            size: 6 + ctx.random.next() * 4,
            maxSize: 18 + ctx.random.next() * 8,
            alpha: 0.5,
            rotation: ctx.random.next() * Math.PI * 2,
            color: [45, 45, 50]
          });

          if (ctx.random.next() < 0.4) {
            ctx.fx.particles.push({
              pos: worldPos.clone(),
              vel: Vector2.fromAngle(ctx.random.next() * Math.PI * 2, 20 + ctx.random.next() * 30).addScaled(ship.vel, 0.4),
              life: 0.2 + ctx.random.next() * 0.2,
              maxLife: 0.4,
              size: 2 + ctx.random.next() * 2,
              color: [255, 110 + ctx.random.next() * 60, 30],
              alpha: 0.9
            });
          }
        }
      ...
      if (ship.system.type === 'MINE_STRIKE' && ship.system.isActive) {
        ...
        if (ship.isPlayer) {
          ...
          const offsetAngle = enemyTarget.facingRad + Math.PI + (ctx.random.next() - 0.5) * 1.0;
          deployPos = enemyTarget.pos.clone().add(Vector2.fromAngle(offsetAngle, 220 + ctx.random.next() * 100));
        }
