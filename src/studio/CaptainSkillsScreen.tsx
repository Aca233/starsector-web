...
const effectLines = (text: string) => text.split("；").map((line, index) => <p key={index}>{line}</p>);
...
function ProfileEditor({ profile, onApply, onClose }: {
  ...
  return <Modal title="舰长档案" description="姓名和头像随当前舰船方案保存，不改变战斗属性。" width="regular" onClose={onClose} footer={<>
    ...
    <fieldset className="captain-portrait-picker"><legend>选择头像</legend>{captainPortraits.map(portrait => <label key={portrait.id} title={portrait.name}>
...
export function CaptainSkillsScreen(props: Props) {
  ...
  const previewId = hoveredId && visibleIds.has(hoveredId) ? hoveredId : effectiveSelectedId;
  const active = tree.skills.find(skill => skill.id === previewId) ?? tree.skills[0];
  ...
  }, [onBack, chosenCount, launchDisabledReason, onLaunch, canUndo, onUndo, active]);
  ...
  const selectSkill = (id: string) => { setHoveredId(null); setSelectedId(id); };
  ...
  const navigateSkill = (event: KeyboardEvent<HTMLButtonElement>, skill: NativeSkill) => {
    ...
    if (next) { setHoveredId(null); setSelectedId(next.id); buttons.current.get(next.id)?.focus(); }
  };
  ...
  return <main className="captain-skills-screen" aria-labelledby="captain-skills-title">
    ...
    <NativeFrame className="captain-character-sheet" surface="glass">
      <div className="captain-sheet-top">
        <aside className="captain-character" aria-label="当前舰长配置">
          <button type="button" className="captain-name-button" onClick={() => setProfileOpen(true)} title="编辑舰长姓名与头像">{profile.name}<span>编辑档案</span></button>
          ...
          <p className="captain-current-design" title={`${props.designName} · ${props.hullName}-级`}>{props.designName} · {props.hullName}-级</p>
          <div className="captain-character-actions">
            ...
            <NativeButton shortcut="G" disabled={!!props.launchDisabledReason} title={props.launchDisabledReason ?? props.launchLabel ?? "使用当前方案和技能进入模拟战斗"} onClick={props.onLaunch}>{props.launchLabel ?? "开始模拟"}</NativeButton>
          </div>
        </aside>
        <section className="captain-skill-detail" style={aptitudeStyle(active.aptitude)} aria-label="技能详情" ref={details} tabIndex={0}>
          <header className="captain-detail-title"><h2>{active.name}</h2><span>{supported ? activeLevel === 2 ? "精英已配置" : activeLevel === 1 ? "普通已配置" : "可配置" : "未接入 · 仅可查看"}</span></header>
          ...
          {definition ? <div className="captain-effects">
            ...
            <h3>精英</h3><div className="captain-elite-effects">{effectLines(definition.elite)}</div>
      ...
      <section className="captain-aptitude-tree" aria-label="四系技能树">
        {tree.aptitudes.map(aptitude => {
          ...
          return <section className="captain-aptitude-row" key={aptitude.id} style={aptitudeStyle(aptitude.id)} aria-label={`${aptitude.name}系技能`}>
            <div className="captain-aptitude-label" title={aptitude.description}><img src={asset(aptitude.icon)} alt="" /><h3>{aptitude.name}</h3></div>
            ...
            <div className="captain-aptitude-icons">
              {entries.map((skill, index) => <button type="button" key={skill.id}
                ...
                title={`${skill.name} · ${levelLabel(skill.id)} · ${definitions.has(skill.id) ? "左键普通→精英，右键取消" : "点击查看资料"}`} onMouseEnter={() => setHoveredId(skill.id)} onMouseLeave={() => setHoveredId(null)}
                onFocus={() => { setHoveredId(null); setSelectedId(skill.id); }} onClick={() => advanceSkill(skill.id)}
    ...
    <footer className="captain-web-tools" aria-label="Web 配置工具">
      <div className="captain-web-toolbar">
        <div className="captain-search"><input ref={search} aria-label="搜索技能" placeholder="搜索技能 /" value={query} onChange={event => setQuery(event.target.value)} />{query && <button type="button" onClick={() => { setQuery(""); search.current?.focus(); }} aria-label="清除搜索">×</button>}</div>
        <select aria-label="筛选技能" value={filter} onChange={event => setFilter(event.target.value as SkillFilter)}><option value="all">全部技能 · 40</option><option value="supported">可配置 · {combatSkillDefinitions.length}</option><option value="configured">已配置 · {chosenCount}</option></select>
        <span className="captain-search-result" role="status">{visibleSkills.length} 项 · 暗色仅供查看</span>
        <div className="captain-web-actions">
          <NativeButton onClick={props.onUndo} disabled={!props.canUndo} title="撤消最近一次技能、档案或装配修改">撤消</NativeButton>
    ...
    {clearOpen && <Modal title="重置全部舰长技能？" description="只清除当前方案的技能，舰长档案、武器、插件和其他装配保持不变。可以撤消恢复。" width="small" onClose={() => setClearOpen(false)} footer={<>
    ...
    </>}><p>已保存的方案副本不会立即改变；再次保存方案后才会覆盖。</p></Modal>}
    {profileOpen && <ProfileEditor profile={profile} onApply={props.onProfileChange} onClose={() => setProfileOpen(false)} />}
    {encyclopedia && <Modal title={`${encyclopedia.name} · 数据百科`} eyebrow="角色技能资料" width="regular" onClose={() => setEncyclopedia(null)} footer={<NativeButton onClick={() => setEncyclopedia(null)}>返回技能</NativeButton>}>
      ...
      {definitions.has(encyclopedia.id) && <div className="captain-encyclopedia-effects"><h3>普通效果</h3><p>{definitions.get(encyclopedia.id)!.normal}</p><h3>精英效果</h3><p>{definitions.get(encyclopedia.id)!.elite}</p></div>}
