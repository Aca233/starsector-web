...
| --- | --- | --- | --- |
| TPC / `WPN-TPC-01` | STATIC_CLOSED for `100×35`, RGBA, ROUGH, `-256`, ppx `1`, muzzle particles, hardpoint-only glow; no synthetic projectile hit glow | deterministic real fire; WebGL/Canvas source strip sampling | fade residual appearance, exact mount flash perception |
...
| Reaper / `WPN-MSL-01` | STATIC_CLOSED for compact body, `fadeTime=0.5`, smoke, engine, red GLOW trail and visual explosion `350 / [255,100,100,255]` | actual straight unguided launch; source explosion contract propagates; direct Canvas regression consumes the live `ContrailEngine` strip even when legacy `engine.contrails` is empty | source kinematics/lifetime; fade residual appearance and detailed explosion-particle runtime execution |
...
| Heavy Blaster / `WPN-HBLASTER-01` | STATIC_CLOSED for beam-like `45×7`, RGBA, scroll/ppx, source `.proj` in-flight glow `35`, independent hit glow `75`, mount/muzzle/audio resources | real armor/hull hit plus direct WebGL/Canvas renderer regression: changing `glowRadius 35 -> 0` removes the 70×70 in-flight halo draw in both paths | source RAY collision/turn-rate differences; exact glow/bloom perception and hit-glow temporal perception |
...
- Canvas and WebGL consume `pixelsPerTexel`; Canvas source strips are sliced/wrapped rather than stretching one complete texture to the requested world length.
...
- Projectile `.proj glowRadius/glowColor` are consumed as an independent in-flight halo in both WebGL and Canvas; this is distinct from weapon-animation `.wpn glowColor` and from impact-only `.proj hitGlowRadius`.
- Persistent missile trails are owned by `ContrailEngine` and consumed by both renderers. Canvas retains legacy point-contrail compatibility but no longer depends on `engine.contrails` for source-authored missile ribbons.
- `.proj hitGlowRadius` owns an independent `hit_glow.png` layer and never creates a seven-frame fireball.
- Missile top-level visual `explosionRadius/explosionColor` own source-authored explosion size/color; the source comment also requires a white additive core above that color, now rendered in both WebGL and Canvas. Exact white-core size/falloff remains `PROVISIONAL`; `explosionSpec.radius/coreRadius` and fuse/damage radii remain outside the visual contract.
