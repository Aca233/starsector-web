...
export const FloatingShipHUD: React.FC<FloatingShipHUDProps> = ({ ship, isEnemy, cameraPosRef, zoomRef, canvasRef }) => {
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      const sx = (ship.pos.x - (cam?.x ?? 0)) * zoom + cw / 2;
      const sy = (ship.pos.y - (cam?.y ?? 0)) * zoom + ch / 2;
      if (ship.isDead || sx < -200 || sx > cw + 200 || sy < -200 || sy > ch + 200) {
  ...
  return (
    <div ref={containerRef} className="hud-floating-tag pointer-events-none absolute top-0 left-0 select-none z-20"
      data-affiliation={isEnemy ? 'enemy' : 'friendly'} style={{ willChange: 'transform', color }}>
