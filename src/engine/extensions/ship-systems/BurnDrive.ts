...
export const burnDrive: ShipSystemDefinition = {
  ...
  advanceAI: ({ship, target, distance, tactical, system = ship.system}) => {
    ...
    const stopping=boostedSpeed*boostedSpeed/(2*stats.deceleration)+boostedSpeed*system.chargeDownDuration;
    ...
    if(system.isActive){
      if(!safe&&system.state!=='OUT')system.deactivate();
    }else if(safe&&aligned&&!system.isCoolingDown)system.activate();
  }
};
