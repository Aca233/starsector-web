...
export function LanTeamControls({room,id,editable,send,openRequest=0}:{room:Room;id:string;editable:boolean;send:(message:unknown)=>void;openRequest?:number}) {
  const [open,setOpen]=useState(false);
  const [lastRequest,setLastRequest]=useState(openRequest);
  if(lastRequest!==openRequest){setLastRequest(openRequest);if(openRequest)setOpen(true);}
  ...
  return <div className="lan-team-controls">
    ...
    <MotionPresence>{open&&<Modal title="房间编队规则" eyebrow="同一房间 · 同队友好，不同队交战" width="small" onClose={()=>setOpen(false)} footer={<NativeButton onClick={()=>setOpen(false)}>返回改装</NativeButton>}>
      <div className="lan-team-rules"><p>当前：{solo?"各自为战，每位玩家和每艘 AI 都独立成队":"自由分队，可组织两队、三队或更多队互打"}。</p>
        ...
        <p>{solo?"新加入的玩家、手动添加的 AI 也会独立成队，不会自动成为他人的队友。切回自由分队后可自行合队。":"左侧成员卡选择 A / B / C 等队伍；房主能调整所有成员。AI 显示在左侧对应队伍，用队名旁的「＋AI」添加。空队不参战。"}</p>
