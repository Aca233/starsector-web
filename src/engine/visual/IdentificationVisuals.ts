...
export const FRIENDLY_INDICATOR_COLOR = [100, 255, 100] as const;
...
export function shipIdentification(ship: Ship, playerSide: number | boolean, alpha: number, combatTime: number): IdentificationDiamond | null {
  // An attached module is part of its carrier/station silhouette, not a separate
  // contact. Keep R-lock/inspection behavior independent of these passive marks.
  if (ship.isAttachedModule || ship.isDead || ship.isRetreated || ship.isDocked) return null;
  const friendly = ship.teamId === (typeof playerSide === "boolean" ? (playerSide ? 0 : 1) : playerSide);
  ...
  return {
    color: friendly ? FRIENDLY_INDICATOR_COLOR : HOSTILE_INDICATOR_COLOR,
