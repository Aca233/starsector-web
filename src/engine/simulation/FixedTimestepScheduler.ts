...
export class FixedTimestepScheduler {
  ...
  public measuredIdlePercent = 82; // 真实 CPU 空闲率百分比 (1:1 测量每秒仿真与渲染耗时)
  ...
  public update(
    ...
    if (this.secondTimer >= 1.0) {
      ...
      // 真实空闲率: (总时间 - 实际执行逻辑与渲染耗时) / 总时间
      const totalElapsedMs = this.secondTimer * 1000;
      const idleFraction = Math.max(0.01, Math.min(0.99, (totalElapsedMs - this.workTimeAccumMs) / totalElapsedMs));
      this.measuredIdlePercent = Math.round(idleFraction * 100);
    ...
    // 消费累加器，执行若干次固定步长逻辑 Tick (单帧最多执行 3 步以防死亡螺旋)
    let steps = 0;
    const maxSubSteps = 3;
